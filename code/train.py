...
def train():
	...
	model = MyChallenge(name=f'BaseChallenge').to(device)
	# model.load_state_dict(torch.load('checkpoints/BaseChallenge/best.pth'))
...
def test():
	...
	model.load_state_dict(torch.load(f'checkpoints/BaseChallenge/best.pth'))
	...
	GT_Predict.to_csv(f'checkpoints/BaseChallenge/TestPredict.csv')
